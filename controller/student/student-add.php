<?php
    ...
    if(!empty($_POST['acctid']) && !empty($_POST['firstname']) && !empty($_POST['email'])){
        ...
        if($fetch = $db->SingleRow("SELECT acad_key, acad_year, sem FROM `semester` WHERE status = 'Active'")){
            ...
            $execute = $db->ExecuteQuery("INSERT INTO accounts (acctno,acctid,email,hashed_password,firstname,middlename,lastname,yearlevel,section,userlevel,otp,acad_key,status,qr_id,encrypt) VALUES ('$acctno','$acctid','$email','$hashed_password','$firstname','$middlename','$lastname','$yearlevel','$section','$userlevel','$otp','$acad_key','$status', uuid(), '$encrypt')");
